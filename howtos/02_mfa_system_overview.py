...
#
# The central class of sodym is the MFASystem class.
...
# ### Flows
#
# An MFA tracks mass flows of materials in a system (like the society of a defined region) though different stages of its lifecycle. In sodym, each of these stages is represented by one Flow object.
...
# The flows are therefore represented as multi-dimensional arrays (NamedDimArrays in sodym).
...
# In sodym, the process is a minimal object, containing only the name and ID of the process.
...
# To represent flows leading in and out of this boundary, the first process in sodym is alwas the 'system environment' process.
...
# This is what parameter objects are for. In sodym, they are also NamedDimArray objects.
#
# ### Scalar parameters
#
# NamedDimArrays can not be zero-dimensional.
...
# In sodym, each of these quantities is represented by a NamedDimArray object.
# Sometimes, lifetime distributions can be used to calculate the outflow from the stock.
# This is handled by the DynamicStockModel class in sodym.
...
# A subset of these dimensions is also stored in each NamedDimArray object.
