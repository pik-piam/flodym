...
def get_example_array() -> NamedDimArray:
    dims = _get_dims()
    return NamedDimArray(dims=dims, name="Example Array", values=np.random.random((31, 3)))
...
def get_example_mfa() -> ExampleMFA:
    ...
    parameters = {
        "eol machines": Parameter(name="eol machines", dims=dims[("t",)], values=np.arange(15.5,31.0,0.5)),
        "eol buildings": Parameter(name="eol buildings", dims=dims[("t",)], values=np.arange(120, 368, 8)),
        "composition eol machines" :Parameter(name="composition eol machines", dims=dims[("e",)], values=np.array([0.8, 0.15, 0.05])),
        "composition eol buildings": Parameter(name="composition eol buildings", dims=dims[("e",)], values=np.array([0.95, 0.045, 0.005])),
        "shredder yield": Parameter(name="shredder yield", dims=dims[("e",)], values=np.array([0.92, 0.1, 0.92])),
        "demolition yield": Parameter(name="demolition yield", dims=dims[("e",)], values=np.array([0.95, 0.02, 0.95])),
        "remelting yield": Parameter(name="remelting yield", dims=dims[("e",)], values=np.array([0.96, 1., 0.5])),
    }
